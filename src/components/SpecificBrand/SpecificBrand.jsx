    ...
    const {data, isError, isLoading} = useQuery({
        ...
        select: (res) => {
            return res.data.data.filter(
                (product) => product.brand.slug.toLowerCase() === brandId.toLowerCase()
            )
        }
    })
    console.log(data);
    ...
    return (
        ...
        {brands?.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center mt-10">
                <h2 className="title pb-10">No products found at the moment</h2>
                <img src={emptyProducts} alt="specific category" className="w-[500px] h-auto object-contain" />
            </div>
        )}
        </>
    )
